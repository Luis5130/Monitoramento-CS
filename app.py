...
if not semanas_do_mes_unicas:
    ...
    for semana_num in semanas_do_mes_unicas:
        ...
        tabela_dados.append({'Mês e Ano': f'--- Semana {semana_num} ---'})
        # Preenche outras colunas com string vazia para manter a estrutura
        for col in metricas_disponiveis:
            tabela_dados[-1][f'Valor ({col})'] = ''
    ...
    if tabela_dados:
        ...
        # Aplica a formatação APENAS a células que não são a linha de cabeçalho (que começa com '---')
        # e APENAS às colunas que precisam de formatação numérica ou percentual.
        st.dataframe(df_final_tabela.style.format(format_dict_combined,
            subset=df_final_tabela.index[~df_final_tabela['Mês e Ano'].astype(str).str.startswith('---')]
        ))
    else:
